...
public final class Alignment {
    ...
    public static double sim(double[] x, double[] w) {
        ...
        scores[0][0] = x[0] * w[0];
        for (i = 1; i < xlen; i++) {
            scores[i][0] = scores[i - 1][0] + x[i] * w[0];
        }
        for (j = 1; j < wlen; j++) {
            scores[0][j] = scores[0][j - 1] + x[0] * w[j];
        }
    ...
    private double computeSimilarity(double[] x, double[] w) {
        ...
        m_scores[0][0] = x[0] * w[0];
        for (i = 1; i < xlen; i++) {
            m_scores[i][0] = m_scores[i - 1][0] + x[i] * w[0];
            m_direction[i][0] = 0;
        }
        for (j = 1; j < wlen; j++) {
            m_scores[0][j] = m_scores[0][j - 1] + x[0] * w[j];
            m_direction[0][j] = 1;
        }
