...
public class MaxMinSoftmax extends Classifier {
    ...
    private double[] activate(Pattern x) {
        ...
        for (int j = 0; j < m_numLabels; j++) {
            ...
            for (int p = 0; p < m_numPartitions; p++) {
                double sim = MinAlignment.sim(x.sequence(), m_W[j][p]);
                if (a[j] < sim) {
                    a[j] = sim;
                }
            }
    ...
    class SSG {
        ...
        SSG(Dataset data) {
            ...
            a = new double[outUnits][numPartitions];
            A = new MinAlignment[outUnits][numPartitions];
        ...
        int fit() {
            ...
            for (int t = 1; t <= T && S == 0; t++) {
                ...
                for (int i = 0; i < numX; i++) {
                    ...
                    // compute activation
                    double[] max_a = new double[outUnits];
                    int[] max_index = new int[outUnits];
                    for (int j = 0; j < outUnits; j++) {
                        for (int p = 0; p < numPartitions; p++) {
                            A[j][p] = new MinAlignment(xi, m_W[j][p]);
                            a[j][p] = A[j][p].sim();
                        }
                        max_index[j] = Array.indexOfMax(a[j]);
                        max_a[j] = a[j][max_index[j]];
                    }
